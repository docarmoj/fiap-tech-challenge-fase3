...
- A renomeacao da routing key exige subir os tres servicos juntos, e exige **`docker compose down -v`** antes do primeiro `up`. Duas razoes, ambas por volume anonimo reaproveitado: o script que cria o database do historico so roda com o PGDATA vazio, e a topologia do RabbitMQ sobrevive ao `down` sem `-v`. Apenas o routing key do evento muda; o `dlq-routing-key` da fila de notificacao continua como estava, de proposito, para nao redeclarar uma fila duravel ja existente com argumento diferente.
- A fila do historico so passa a existir quando o carehub-historico sobe pela primeira vez. Consulta criada antes disso e publicada sem destino no historico e se perde sem rastro. Nao ha alternate exchange; o contorno e subir o historico junto com o agendamento.

### Limites conhecidos, aceitos nesta entrega

- **`ocorridoEm` e carimbado pelo publisher depois do commit.** Duas alteracoes concorrentes na mesma consulta podem ser publicadas fora da ordem em que foram gravadas, e a mais antiga vencer no historico. Fechar isso exigiria uma coluna de versao em `tb_consulta`, no agendamento. A janela e estreita e o projeto nao tem escrita concorrente na demonstracao.
- **O enum de status e duplicado sem teste de sincronia.** Um valor novo no agendamento vira mensagem venenosa no historico: excecao, tres tentativas e DLQ, com o read model congelado no status anterior. Ninguem observa a DLQ hoje.
- **O upsert e read-then-write e depende de concorrencia 1 no listener.** Aumentar `spring.rabbitmq.listener.simple.concurrency` faria dois eventos da mesma consulta colidirem na insercao. A configuracao atual nao define concorrencia, entao vale o padrao 1.
- **A carga inicial do historico amarra os ids por literal.** Este banco nao tem os cadastros de paciente e de profissional, entao nao ha chave natural para resolver, como o `V2` do agendamento faz. Os dois seeds so concordam enquanto as sequences do agendamento produzirem 1..n na ordem atual. Mexer na ordem daquele seed desalinha os vinculos das credenciais deste. Documentado no cabecalho da migration.
- **`ocorridoEm` nao tem guarda de nulo.** Mensagem republicada a mao pelo painel do RabbitMQ sem esse campo estoura no listener e termina na DLQ.
